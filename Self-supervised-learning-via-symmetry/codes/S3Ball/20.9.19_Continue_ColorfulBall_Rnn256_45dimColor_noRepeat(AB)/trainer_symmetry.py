...
def symm_rotaY_colorful_ball(z, rotator):
    z_do = z[..., 0:3]
    z_rest = z[..., 3:]
    z_change = torch.matmul(z_do.unsqueeze(1), rotator).squeeze(1)
    return torch.cat((z_change, z_rest), -1)
...
class BallTrainer:
    ...
    def train(self):
        ...
        for i in range(iter_num, self.max_iter_num):
            ...
            z_gt, mu, logvar = self.model.batch_seq_encode_to_z(data)
            T, Tr = make_translation_batch(batch_size=BATCH_SIZE * self.t_batch_multiple,
                                           dim=np.array([1, 0, 1, 0, 0]), t_range=self.t_range)
            ...
            R_z_loss = self.batch_symm_z_loss(
                ...
                lambda z: symm_rotaY_colorful_ball(z, R), lambda z: symm_rotaY_colorful_ball(z, Rr)
            )
            ...
            R_recon_loss = self.batch_symm_recon_loss(
                data[:, 1:, :, :, :], z_gt,
                R_sample_points, self.t_batch_multiple,
                lambda z: symm_rotaY_colorful_ball(z, R), lambda z: symm_rotaY_colorful_ball(z, Rr)) if self.enable_SRSD else torch.zeros(1)
